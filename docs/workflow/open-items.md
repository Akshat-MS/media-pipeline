...
| DEC-002 | **Layer 5's `semantic_type` comes from a fixed list; the list constrains MEANING, not FORM.** Slide 3's connector `v017_s03_84` and slide 5's code line `next_produced ----> buffer` both mean *data moves into the buffer* and both resolve to one term. Without a list, Layer 5 invents a name per context — one video would carry two names for one idea, Layer 6 would make two bindings, and the same event would appear in two colours ninety seconds apart. Across five videos the same relationship recurs as producer→buffer, writer→shared-data and Pᵢ→Rⱼ; three ad-hoc names would defeat the project-level binding that exists to stop exactly that drift | The list lives in the layer file. A video may add a term only by naming why no existing one fits. **It binds the colour and its redundant channel — never the shape.** Form comes from Layer 3's `type`, so an arrow stays an arrow and a code line stays a code line. The list is a starting vocabulary, corrected once the video exists; a correction is one row in a binding table, not a rename across every artifact | Layer 6's binding; cross-video consistency | — | **decided** |
| PROP-003 | **Nothing forbids the pipeline INVENTING content.** VGR-01 stops notation being dropped; VGR-07 stops narrated points being dropped from the plan. Both guard against loss. Neither looks at what arrives that was never in the deck or the narration — and an invented element carries the same authority as a real one, with nobody positioned to notice. Three cases, hardest last: a circular buffer with head/tail pointers (never mentioned); concrete counts like *3 of 5 occupied* when the professor only ever says *n elements*; and — the dangerous one — a renderer reading `v017_s05_104`'s *"If  0, waits"* beside its mirror *"If > 0, adds to buffer"* and rendering **`If == 0`**. That is an invented CORRECTION. It looks right, it looks better than the source, and it silently overwrites the one artifact that deliberately preserved the defect for a human to settle (OBS-033) | Proposed rule: every visual element must trace to the deck or the narration; anything with no source is **named as an addition** and approved deliberately. It forbids nothing — it makes the list visible, and a correction appearing on an additions list is exactly what OBS-033 says must not happen unreviewed. **Deferred to Layer 6 by decision** — decide the mechanism when Directions are actually being generated and the cost is concrete | Every layer that renders; OBS-033 | Medium | open — accepted in principle, mechanism deferred to Layer 6 |
| DEC-003 | **The Layer 5 manual review is skipped for V017.** 45 elements is too many to work through by hand, and the mechanical checks now pass with 0 failures. Decided: proceed without a human pass | **The artifact must say so.** `metadata.review` is set to `skipped`, not `confirmed` — the same distinction as `skim` versus `ok` in the transcript review: one says a human checked and was satisfied, the other says nobody looked. A representation nobody reviewed is a set of proposals, and if a wrong meaning surfaces in Layer 8 this row is where the answer to *was this checked?* lives. **The reviewable set is far smaller than 45** — 22 chrome elements are replaced by the style contract, 9 have no narration, and 8 are structural labels that change no picture. Six carry domain meaning; that is the set to revisit if anything downstream looks wrong | Confidence in Layer 5's output | Medium | open — deliberate gap |
| OBS-040 | **`counter` and `capacity` were one term, and the definition contradicted itself.** `counter` read *"a number that changes (empty, full, **n**, Available)"* — listing `n`, the one quantity in the set that never changes. The first real run resolved `BUFSIZE` as `counter`, correctly obeying a wrong definition. Left alone, Layer 6 would bind a fixed capacity to whatever treatment it gives a changing number, and animate a constant | Fixed. `capacity` split out: *a fixed size that does NOT change (BUFSIZE, n)*. `counter` is now *a number that goes up and down*. Added to the tool's fixed list and to the layer file (v1.1). **The prompt was right and the vocabulary was wrong** | Layer 6 binding | Medium | **closed** |
| OBS-041 | **The focus-arrow detector picked up logos.** Three of twenty extracted runs were the KLE mark (red) and the ekLakshya dots (orange) in the intro, not the focus arrow. They then read as the focus map *disagreeing with ground truth* — when the ground truth was the thing that was wrong. The real score was 17 of 17 genuine runs agreeing | Fixed. Detection is restricted to windows that matched a deck slide: the arrow lives on the slides, and everywhere else orange is a logo. **Re-check on V018** — the colour threshold is still tuned to orange-on-purple (OBS-038) | Layer 5 verification | Medium | **closed** |
