...
def test_checks_for_stage_groups_correctly():
    ...
    assert checks == {"sync_drift", "render_fps", "duration_match", "output_integrity"}
...
def test_adaptive_fps_inherits_source():
    assert get_required_fps(24.0) == 24.0


def test_adaptive_fps_falls_back_when_no_source():
    assert get_required_fps(None) == 30.0


def test_render_fps_gate():
    ok = validate_render_fps(24.0, get_required_fps(24.0))
    low = validate_render_fps(20.0, get_required_fps(24.0))
